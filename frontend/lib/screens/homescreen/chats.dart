...
class _ChatsScreenState extends State<ChatsScreen> {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text('Ingresa el correo del contacto:',
  ...
  Future<void> _abrirChatPorCorreo(BuildContext dialogCtx) async {
    ...
    final chatId = await _chatService.openOrCreateChat(
      user['uid'],
      otherName: (user['name'] ?? user['nombre'] ?? user['email']).toString(),
    );
    Navigator.pop(dialogCtx);
...
class _ChatTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final userName = (participantNames[otherUid] ?? 'Chat SOS').toString();
    return Builder(
      builder: (context) {

...
class _MapsScreenState extends State<MapsScreen>
  ...
  final List<Map<String, dynamic>> _lugaresSeguros = [
    {'nombre': 'Hospital Más Cercano', 'icono': Icons.local_hospital, 'tipo': 'hospital'},
    {'nombre': 'Policía Más Cercana',  'icono': Icons.local_police,   'tipo': 'police'},
  ...
  void initState() {
    ...
    _actualizarHora();
    _clockTimer = Timer.periodic(const Duration(seconds: 30), (_) => _actualizarHora());
    _iniciarGPS();
  }
  ...
  Future<void> _calcularRuta(ll.LatLng destino, String nombre) async {
    ...
    try {
      ...
      _mostrarSnack('⏱️ La solicitud tardó demasiado, intenta de nuevo', error: true);
    } catch (e) {
      setState(() => _calculandoRuta = false);
  ...
  Future<void> _buscarLugar(String query) async {
    ...
    try {
      ...
      _mostrarSnack('⏱️ Búsqueda tardó demasiado, intenta de nuevo', error: true);
    } catch (_) {
      setState(() => _buscando = false);
    }
  }
                  ...
                  builder: (context, snap) {
                    ...
                    return BottomNavigationBar(
                      ...
                      onTap: (index) {
                        if (index == 2) return;
                        const routes = [
                          '/profile', '/guardian', '', '/chats', '/notices'
                        ];
